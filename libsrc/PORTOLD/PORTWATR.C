...
{
   if (plane->norm->raw.x == 0.0)
      return;

   v->x = (-(plane->norm->raw.y * v->y)
          - (plane->norm->raw.z * v->z)
          - (plane->plane_constant)) / plane->norm->raw.x;
}

static void project_vector_y_onto_zx(mxs_vector *v, PortalPlane *plane)
{
   if (plane->norm->raw.y == 0.0)
      return;

   v->y = (-(plane->norm->raw.z * v->z)
          - (plane->norm->raw.x * v->x)
          - (plane->plane_constant)) / plane->norm->raw.y;
}

static void project_vector_z_onto_xy(mxs_vector *v, PortalPlane *plane)
{
   if (plane->norm->raw.z == 0.0)
      return;

   v->z = (-(plane->norm->raw.x * v->x)
          - (plane->norm->raw.y * v->y)
          - (plane->plane_constant)) / plane->norm->raw.z;
}
