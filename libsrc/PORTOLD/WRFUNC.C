...
// $Header: r:/t2repos/thief2/libsrc/portold/wrfunc.c,v 1.17 1998/03/16 12:28:43 MAT Exp $
...
{
   mxs_real distance;
   wrBspNode *BSP = wrBspTree;
   mxs_vector *point = &loc->vec;

   while (BSP->leaf == FALSE) {
      distance = mx_dot_vec(point, &BSP->plane.v) + BSP->plane.d;
      if (distance < 0.0)
         BSP = BSP->outside;
      else
         BSP = BSP->inside;
...
#ifndef SHIP
   if (!wr_num_cells) {
      Warning(("You can't find a Location when there's no world rep!\n"));
...
{
   return mx_dot_vec(&p->norm->raw, &loc->vec) + p->plane_constant;
}
...
void PortalMovePointInsideCell(Location *loc)
{
   ...
   do {
      ...
      for (i=0; i < n; ++i,++p) {
         ...
         if (dist < TOO_CLOSE) {
            mx_scale_addeq_vec(vec, &p->norm->raw, ADJUST_DIST-dist);
            changed = TRUE;
         }
      }
      ++iter;
           ...
           mxs_vector *mn, mxs_vector *mx, int vl)
{
   ...
   if (plane->norm->raw.x < 0)
      rel_min.x = mx->x, rel_max.x = mn->x;
   else
      rel_min.x = mn->x, rel_max.x = mx->x;

   if (plane->norm->raw.y < 0)
      rel_min.y = mx->y, rel_max.y = mn->y;
   else
      rel_min.y = mn->y, rel_max.y = mx->y;

   if (plane->norm->raw.z < 0)
      rel_min.z = mx->z, rel_max.z = mn->z;
   else
      rel_min.z = mn->z, rel_max.z = mx->z;

   min_dist = mx_dot_vec(&rel_min, &plane->norm->raw) + plane->plane_constant;
   max_dist = mx_dot_vec(&rel_max, &plane->norm->raw) + plane->plane_constant;
...
#define CELL_VISITED(x)       (WR_CELL(x)->render_data)
#define SET_CELL_VISITED(x)   (CELL_VISITED(x) = (void *) 1)
#define CLEAR_CELL_VISITED(x) (CELL_VISITED(x) = 0)
...
int PortalBlockVisionFromLocation(Location *seed_loc)
{
   ...
   if (cell_index == CELL_INVALID) {
      mprintf("Door blocking out of world rep: (%g %g %G)\n",
...
int PortalUnblockVisionFromLocation(Location *seed_loc)
{
   ...
   if (cell_index == CELL_INVALID) {
      mprintf("Door unblocking out of world rep: (%g %g %G)\n",
              seed_loc->vec.x, seed_loc->vec.y, seed_loc->vec.z);
      return 0;
   } else
